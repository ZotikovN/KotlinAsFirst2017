...
fun diameter(vararg points: Point): Segment {
    ...
    return Segment(points[m], points[n])
}
/**
 * Простая
 *
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        val y = ((Math.sin(other.angle) * b)
                - (Math.sin(angle) * other.b)) / (Math.sin(other.angle - angle))
        val x = ((Math.cos(angle) * other.b)
                - (Math.cos(other.angle) * b)) / (Math.sin(angle - other.angle))
        val ans = Point(x, y)
        return ans
    }
...
fun lineBySegment(s: Segment): Line =
        Line(s.begin, Ang(Math.atan2((s.end.y - s.begin.y), (s.end.x - s.begin.x))))
...
fun bisectorByPoints(a: Point, b: Point): Line {
    val x = Point((a.x + b.x) / 2, (a.y + b.y) / 2)
    val m = Math.PI / 2 + lineByPoints(a, b).angle
    val y = Ang(m)
    return Line(x, y)
}
